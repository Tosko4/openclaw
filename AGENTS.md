...
- For regression audits or changes to existing behavior, read [regression checks](.agents/references/regression-review/README.md). Complete contract-relevant proof and the chosen workflow's required gates within user/host limits; command references do not mandate unrelated suites. Reuse valid proof; rerun for changed inputs or missing coverage. Docs-only work needs docs sanity and `git diff --check`. Report unrun checks and gaps.

...
- `logging.redactPatterns`: array of regex strings (replaces the default string list). Built-in structural protections for secret-bearing log fields, form bodies, structured authorization headers, and bare AWS secret access keys always apply. File and JSON console records apply these protections before JSON encoding.

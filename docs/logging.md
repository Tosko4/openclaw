...
- `logging.redactPatterns`: list of regex strings that replaces the default string list for log/transcript output. Built-in structural protections for secret-bearing log fields, form bodies, structured authorization headers, and bare AWS secret access keys always apply, including when this list is copied or customized. A shared JSON-aware owner masks file and JSON console writes and applies the current policy when reading stored JSON logs. Configured patterns match original decoded values and serialized context, including context across fields and arrays. Explicit patterns also apply to shell references and internal session paths. JSON records use `***` masks; captured numbers, booleans, and null become quoted masks. JSON structure stays valid, and already-masked output stays stable when tailed. Raw-text output retains its existing mask format. For Control UI tool payloads, custom patterns apply on top of the built-in defaults, so adding a pattern never weakens redaction of values already caught by the defaults.

...
- The picker lists models permitted by the allow list. **Default** clears the session pin and shows the effective automatic model, including a configured fallback when the primary is no longer listed. Automatic fallbacks are not manual choices unless the allow list permits them.

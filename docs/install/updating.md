...
they name the same operation-scoped helper before and after its move out of the
staging prefix. The staging invocation includes `--anchor` to select the recorded
journal. The helper verifies its own recorded path, identity and content before
using that journal. It is outside the live package and disposable recovery
...
receipt remains outside that directory and is readable through

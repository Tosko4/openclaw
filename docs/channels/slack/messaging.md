...
In `progress` mode, Slack's native agent card is the default: the whole turn is one streamed message that interleaves narration with a live plan/task card and finishes with the assistant's answer in that same message. The card shows authored plan steps when the agent publishes a plan, otherwise one stable work-summary row; approval requests get their own row. Quiet progress omits intermediate command-failure telemetry before publishing native rows; it still preserves blocked states, unrecognized outcomes, and unrelated tool errors. With `progress.toolProgress: true`, it also shows per-tool task rows alongside any authored plan. Routine updates coalesce at one-second intervals; approvals, failures, and completion bypass that delay. With the tool log enabled, command failures remain visible and settle as `Recovered: …` when the turn completes successfully. This label reflects turn completion, not a verified retry of each command. The card appears only once a turn does real work — tool or plan activity still running after a short delay — so a plain question is answered without one.
...
Set `channels.slack.streaming.progress.style` to `"compact"` for one plain-text progress draft instead of either card surface. Explicitly setting `progress.toolProgress: false` also selects compact style when `style` is unset; leaving both options unset keeps the default quiet card. Set `style: "card"` to keep a card with `toolProgress: false`. Commentary appears as italic text; authored reasoning, approval requests, and blocked states remain visible. Quiet compact progress omits the same intermediate command failures as the cards; it does not show plan checklists. The final response is posted as a new message, then the temporary preview is deleted after Slack confirms delivery. Older previews displaced by human replies are cleaned up with it; durable messages and videos stay in the conversation.

This quiet-mode behavior also applies when `toolProgress` is omitted or inherited from `streaming.preview.toolProgress: false`. It changes older releases that retained every failed command in quiet cards. Set `streaming.progress.toolProgress: true` to retain the command log. Final error replies and the unconditional warning when a failed tool would otherwise leave no reply are unaffected; command outcomes remain in agent context and logs.

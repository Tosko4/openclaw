...
describe("updater plugin commit cancellation", () => {
  ...
  it("rechecks the original owner after the awaited persistence hook", async () => {
    await withOpenClawTestState({ label: "plugin-composed-authority" }, async (state) => {
      const cfg = { plugins: { enabled: false } };
      await state.writeConfig(cfg);
      const originalConfig = await fs.readFile(state.configPath, "utf8");
      await writePersistedInstalledPluginIndexInstallRecords({}, { config: cfg, env: state.env });
      const controller = new AbortController();
      const refusal = new Error("original owner revoked during persistence hook");
      const beforePersistentEffect = vi.fn(async () => {
        await Promise.resolve();
        controller.abort(refusal);
      });
      mocks.convergence.mockResolvedValueOnce({
        changes: [],
        warnings: [],
        errored: false,
        smokeFailures: [],

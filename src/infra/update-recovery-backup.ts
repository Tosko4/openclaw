...
import {
  ...
  type UpdateRecoveryRestoreResult,
...
async function prepareVerifiedBackup(ref: UpdateRecoveryBackupRef) {
  ...
  try {
    ...
    return {
      ...
      payloads,
      close,
      assertCurrent: () => directoryPin.assertCurrent(),
    };
  } catch (error) {
    await close();
    throw error;
...
export async function restoreUpdateRecoveryBackup(
  ...
  const prepared = await prepareVerifiedBackup(ref);
  let restored = false;
  let result: UpdateRecoveryRestoreResult = { warnings: [] };
  try {
    await withConfigMutationLock({}, async () => {
      const { getUpdateRunForRecovery } = await import("./update-run-reader.js");
      const previousRun = await getUpdateRunForRecovery(prepared.manifest.runId);
      ...
      try {
        result = await restorePreparedUpdateRecoveryBackup(
          {
            ...prepared,
            assertCurrent: async () => {
      ...
      try {
        ...
        if (!currentCapture) {
          recordUpdateRunRecoveryCapture(previousRun.runId, savedCapture, authority.assertOwned);
        ...
        if (restoreFailure) {
          throw new AggregateError(
            ...
            `State restoration failed and update run receipts could not be recovered. Capture retained at ${ref.manifestPath}; keep the Gateway stopped and inspect the retained artifacts before manual recovery.`,
            { cause: error },
          );
        }
        throw error;
    ...
    authority.assertOwned();
    log.warn(
      `State restored; update receipts could not be completed for ${ref.manifestPath}: ${formatErrorMessage(error)}`,
    );
  } finally {
...
> {
  ...
  for (const captureId of await fs.readdir(store)) {
    ...
    if (await statOrMissing(terminalPath)) {
      ...
      const capture = (await getUpdateRunForRecovery(manifest.runId))?.origin.updateRecoveryCapture;
...
export async function assertNoUnresolvedUpdateRecoveryBackup(
  ...
  if (existing) {
    throw new Error(
      `Update capture ${existing.ref.manifestPath} remains retained (${existing.outcome.status}); another protected mutation is refused. Inspect with openclaw update status --json and resolve the retained capture before retrying.`,
    );
  }
}
...
export async function inspectUpdateRecoveryBackups(params: { installRoot?: string } = {}) {
  ...
  return await Promise.all(
    snapshots.map(async ({ ref, manifest, outcome }) => {
      ...
      const retainedCandidate = (await fs.readdir(ref.directory)).find((name) =>
        name.startsWith(".restore-"),
      );
      try {
        const run = await getUpdateRunForRecovery(manifest.runId);
      ...
      if (retainedCandidate) {
        terminalOutcome = undefined;
        ambiguity = `candidate publication artifacts remain at ${path.join(ref.directory, retainedCandidate)}`;
      }
      ...
      return {
        ...
        terminalOutcome,
        nextAction,
        message: retainedCandidate
          ? `Update recovery set ${ref.manifestPath}: ${reason}. Keep the Gateway stopped and inspect both baseline and candidate artifacts before manual recovery; automatic retry is refused.`
          : `Update recovery set ${ref.manifestPath}: ${reason}. ${status === "unresolved" ? "Keep the Gateway stopped and run" : "Automatic restoration is refused; inspect with"} \`${nextAction}\`.${status === "ambiguous" ? " Resolve the recorded outcome before retrying `npx openclaw@latest doctor --fix`." : ""}`,
      };
    }),
  );
}

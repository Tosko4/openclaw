...
export async function runUpdateRepairWorker(
  ...
  let finalValidation: UpdateRepairValidation = {
    ok: false,
    score: 0,
    summary: "Update repair worker did not validate the installation.",
  };
  ...
  if (params.isCurrent && !params.runId) {
    return stopped(
      "unavailable",
      "Update repair requires the admitting update run identity to preserve its execution guard.",
    );
  }
    ...
    try {
      ...
      if (message.type === "ready") {
        ...
        if (params.context.phase === "validating" && !message.candidateRehearsal) {
          throw new Error(
            "This version cannot repair an isolated update. Run openclaw triage to inspect the validation failure.",
          );
        }
        started = true;
        ...
        if (!started || pending || message.id !== ++requestId || requestId > budget.maxTurns + 1) {
          throw new Error("Update repair validation request is outside its active turn.");
        }
        ...
        if (pending?.id === message.id) {
          pending.controller.abort(new Error("Update repair validation was cancelled."));
        }
        ...
        if (
          ...
          throw new Error("Update repair exceeded its turn budget.");
        }
        if (message.event.type === "turn-finished") {
          if (attempts.length >= budget.maxTurns || message.event.turn !== attempts.length + 1) {
            throw new Error("Update repair repeated a completed turn.");
          }
  ...
  try {
    ...
    pending?.controller.abort(new Error("Update repair worker exited."));

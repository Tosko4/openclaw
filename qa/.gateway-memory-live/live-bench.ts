...
import { delay, stopChild } from "../../scripts/lib/gateway-bench-child.ts";
import {
  controlGatewayHeapProfile,
  readGatewayHeapProfile,
  type GatewayHeapProfile,
} from "../../scripts/lib/gateway-bench-heap.ts";
import { getFreePort, readProcessRssMb } from "../../scripts/lib/gateway-bench-probes.ts";
import {
...
import {
  ...
  captureLiveEvent,
  requireLiveBudget,
  captureLiveHistories,
  finishLiveProof,
  recordMeasuredLiveProof,
  selectLiveCleanupSessionKeys,
...
type BenchmarkRun = {
  ...
  unobservedModelRequestCount: number;
  probeWarmup: {
    durationMs: number;
...
async function runTurn(
  ...
  acceptedTurns.push({
    ...
    sessionKey: options?.sessionKey ?? `agent:main:gateway-concurrency-${index + 1}`,
    expected: `LIVE_GATEWAY_OK_${index + 1}`,
  });
  options?.onStarted?.();
...
async function runGatewaySample(options: {
  ...
  const [port, mockPort, inspectorPort] = await Promise.all([
    getFreePort(),
    getFreePort(),
    getFreePort(),
  ]);
  ...
  const cleanupProof = {
    mode: supportedCleanup ? "supported-session-deletion" : "allocation-only",
    intendedDeletes: supportedCleanup ? options.sessionCount - options.concurrency : 0,
    completedDeletes: 0,
    passed: supportedCleanup ? false : null,
    error: null,
    durationMs: 0,
  };
  ...
  try {
    try {
      ...
      const gatewayArgs = buildGatewayBenchChildArgs(options.entry, port, [
        ...
        "--import",
        new URL("./live-http-counts.mjs", import.meta.url).href,
        "--import",
      ...
      profiler = await connectAllocationProfiler(
        inspectorPort,
        process.env.OPENCLAW_PERF_PROFILE_DIR,
        gateway,
      );
      ...
      if (readyz.length !== 100 || history.length !== 1200 || sessionUpdates.length !== 100)
        throw new Error("Fixed workload count mismatch");
      if (
        [
          freshConnectionResult,
          ...readyz,
          ...controlUi,
          ...history,
          ...sessionUpdates,
        ...
        throw new Error("Fixed workload contains failed probes");
      ...
      const unobservedModelRequestCount = existsSync(requestLogPath)
      ...
      try {
        if (profiler && result) {
          writeFileSync(
            path.join(process.env.OPENCLAW_PERF_PROFILE_DIR, "measured-workload.json"),
            JSON.stringify({ result, allocationPhases }, null, 2),
            { mode: 0o600 },
          );
          if (supportedCleanup) {
            await delay(3000);
            await profiler.snapshot("disconnected-idle");
            const cleanupClient = await connectGateway(
              port,
              performance.now() + 1200000,
              protocolVersion,
              false,
            );
            try {
              for (const key of selectLiveCleanupSessionKeys(options.sessionCount, acceptedTurns)) {
                const deletion = await cleanupClient.request("sessions.delete", {
                  key,
                  deleteTranscript: true,
                });
                if (!deletion?.deleted)
                  throw new Error(
                    "Owned synthetic session cleanup did not delete expected session",
                  );
                cleanupProof.completedDeletes++;
                if (cleanupProof.completedDeletes % 100 === 0)
                  console.error(
                    JSON.stringify({
                      evidence: "supported-cleanup-progress",
                      completedDeletes: cleanupProof.completedDeletes,
                    }),
                  );
              }
            } finally {
...
function summarizeRuns(
  ...
  return {
    ...
    unobservedModelRequestCount: runs.reduce(
      (count, run) => count + run.unobservedModelRequestCount,
      0,
    ),
...
async function main(): Promise<void> {
  ...
  const payload = {
    ...
    mode: "live-openai-allocation-gateway",
  ...
  if (runs.some((run) => !run.liveProof?.passed))
    throw new Error("Live response/transcript proof failed");
  if (runs.some((run) => run.gatewayExit?.exitCode !== 0 || run.gatewayExit?.signal !== null))
    throw new Error("Live Gateway did not exit cleanly");
  if (
    runs.some(
      (run) =>
        run.cleanupProof.error ||
        (run.cleanupProof.mode !== "allocation-only" && !run.cleanupProof.passed),
    )
  )
    throw new Error("Cleanup or evidence capture failed; see recorded counts");
